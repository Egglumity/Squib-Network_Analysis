#!/usr/bin/env python3
"""
main11.py - Network Analysis Tool (main10 -> main11)

Adds:
- Export all (ZIP): JSON + PDF + config + changelog
- Simple Dark Mode (built-in palette)
- Status bar with Next scan countdown, Last scan time, Suspicious log count
- Notifications dock + toast popup on scan completion
- Documentation generation helpers (README and CHANGELOG templates written to disk)
- Config-driven auto-scan & log rotation
"""
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Notifications dock
        self.notifDock = QtWidgets.QDockWidget("Notifications", self)
        ...
        # status widgets (Next scan countdown, Last scan, Suspicious count)
    ...
    def _setup_auto_scan(self):
        ...
        self.auto_timer.start(60 * 1000)  # tick each minute to check
        self.next_scan_time = time.time() + minutes * 60

    def _auto_scan_trigger(self):
        # If it's time, run a scan
    ...
    def _on_scan_done(self, snap: Snapshot):
        ...
        # If auto scan triggered, export
        if self.config.get("enable_auto_scan", False):
            # Export JSON and cleanup
            try:
    ...
    def _populate_from_snapshot(self, snap: Snapshot):
        ...
        # Notification list update for suspicious
        if snap.suspicious:
            for s in snap.suspicious:
    ...
    def on_export_all(self):
        ...
        try:
            Path('config.json').exists() and cfg_path.write_text(Path('config.json').read_text())
        except Exception:
            pass
        # changelog entry
